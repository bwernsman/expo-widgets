...
const withAppGroupPermissions: ConfigPlugin<WithExpoIOSWidgetsProps> = (
    ...
    return withEntitlementsPlist(
        config,
        (newConfig: ExportedConfigWithProps<Record<string, any>>) => {
            ...
            if (modResultsArray.indexOf(entitlement) !== -1) {
                Logging.logger.debug(
                    `Adding entitlement ${entitlement} to config`,
                );
                return newConfig;
            }
            ...
            newConfig.modResults['aps-environment'] =
                getPushNotificationsMode(options);
...
const withAppExtensions: ConfigPlugin<WithExpoIOSWidgetsProps> = (
    config: ExpoConfig,
    options: WithExpoIOSWidgetsProps,
) => {
...
const withLiveActivities: ConfigPlugin<WithExpoIOSWidgetsProps> = (
    config: ExpoConfig,
    options: WithExpoIOSWidgetsProps,
) => {
    config.ios = {
        ...
        infoPlist: {
            ...
            NSSupportsLiveActivitiesFrequentUpdates:
                options?.frequentUpdates || false,
        },
    };

    return config;
};
export const withConfig: ConfigPlugin<WithExpoIOSWidgetsProps> = (
    config: ExpoConfig,
    options: WithExpoIOSWidgetsProps,
) => {
